...
internals.Any.prototype.withvalue = function (testValue) {
    ...
    this._dependency('withvalue', peers, function (value, state, options) {
        if(testValue !== value) {
            return null;
        }

        if (!state.parent) {
            return Errors.create('any.withvalue.parent', null, state, options);
        }
        ...
        for (var i = 0, il = peers.length; i < il; ++i) {
            if (!state.parent.hasOwnProperty(peers[i]) ||
                ...
                return Errors.create('any.withvalue.peer', { value: peers[i] }, state, options);
            }
        }
